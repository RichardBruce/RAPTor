...
inline void float_flip_inplace(unsigned long *const f)
{
    unsigned long mask = -static_cast<long>((*f) >> 63) | 0x8000000000000000;
    (*f) ^= mask;
}

inline unsigned long inverse_float_flip(unsigned long f)
{
...
{
    static_assert(sizeof(unsigned long) == sizeof(double), "Error: Double and unsigned int dont have the same size");
    unsigned long *sort = reinterpret_cast<unsigned long*>(sorted);
    unsigned long *array = reinterpret_cast<unsigned long*>(farray);

    const int histogram_size = 2048;
    unsigned int hist0[histogram_size * 6];
    unsigned int *hist1 = hist0 + histogram_size;
    unsigned int *hist2 = hist1 + histogram_size;
    unsigned int *hist3 = hist2 + histogram_size;
    unsigned int *hist4 = hist3 + histogram_size;
    unsigned int *hist5 = hist4 + histogram_size;
    memset(hist0, 0, histogram_size * 6 * sizeof(unsigned int));
    ...
    for (int i = 0; i < elements; ++i)
    {
        ...
        ++hist0[ array[i]        & 0x7ff];
        ++hist1[(array[i] >> 11) & 0x7ff];
        ++hist2[(array[i] >> 22) & 0x7ff];
        ++hist3[(array[i] >> 33) & 0x7ff];
        ++hist4[(array[i] >> 44) & 0x7ff];
        ++hist5[(array[i] >> 55) & 0x7ff];
    }
    ...
    for (int i = 0; i < histogram_size; ++i)
    {
        ...
        const unsigned int tmp3 = hist3[i] + sum3;
        hist3[i] = sum3 - 1;
        sum3 = tmp3;

        const unsigned int tmp4 = hist4[i] + sum4;
        hist4[i] = sum4 - 1;
        sum4 = tmp4;

        const unsigned int tmp5 = hist5[i] + sum5;
        hist5[i] = sum5 - 1;
        sum5 = tmp5;
    }
    ...
    for (int i = 0; i < elements; ++i)
    {
        const unsigned long data = array[i];
        const unsigned long pos = data & 0x7ff;
        sort[++hist0[pos]] = data;
    }
    ...
    for (int i = 0; i < elements; ++i)
    {
        const unsigned long data = sort[i];
        const unsigned long pos = (data >> 11) & 0x7ff;
        array[++hist1[pos]] = data;
    }
    ...
    {
        const unsigned long data = array[i];
        const unsigned long pos = (data >> 22) & 0x7ff;
        sort[++hist2[pos]] = data;
    }

    /* Move based on histogram 3 */
    for (int i = 0; i < elements; ++i)
    {
        const unsigned long data = sort[i];
        const unsigned long pos = (data >> 33) & 0x7ff;
        array[++hist3[pos]] = data;
    }

    /* Move based on histogram 4 */
    for (int i = 0; i < elements; ++i)
    {
        const unsigned long data = array[i];
        const unsigned long pos = (data >> 44) & 0x7ff;
        sort[++hist4[pos]] = data;
    }

    /* Move based on histogram 5 */
    for (int i = 0; i < elements; ++i)
    {
        const unsigned long data = sort[i];
        const unsigned long pos = data >> 55;
        array[++hist5[pos]] = inverse_float_flip(data);
    }
}
