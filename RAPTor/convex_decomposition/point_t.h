...
class point_t
{
    public :
        ...
        explicit operator point_ti()            const;
...
inline float trapezium_volume(const point_t &a, const point_t &b, const point_t &c, const point_t &d)
{
...
class point_ti
{
    public :
        ...
        int y;
        int z;
        point_ti(const int x = 0, const int y = 0, const int z = 0) : x(x), y(y), z(z) { };
        
        /* Element access */
        inline       int &     operator[](const int i)                  {       int* a = &this->x; return a[i];                                     }
        inline const int &     operator[](const int i) const            { const int* a = &this->x; return a[i];                                     }

        /* Unary operators */
        inline const point_ti   operator-()     const                   { return point_ti(-this->x, -this->y, -this->z);                            }
        ...
        inline const point_ti&   operator+=(const point_ti &rhs)        { this->x += rhs.x; this->y += rhs.y; this->z += rhs.z; return *this;       }
        inline const point_ti&   operator-=(const point_ti &rhs)        { this->x -= rhs.x; this->y -= rhs.y; this->z -= rhs.z; return *this;       }
        inline const point_ti&   operator*=(const point_ti &rhs)        { this->x *= rhs.x; this->y *= rhs.y; this->z *= rhs.z; return *this;       }
        inline const point_ti&   operator/=(const point_ti &rhs)        { this->x /= rhs.x; this->y /= rhs.y; this->z /= rhs.z; return *this;       }

        /* With int */
        inline const bool       operator==(const int rhs)               { return ((this->x == rhs) && (this->y == rhs) && (this->z == rhs));        }
        inline const bool       operator!=(const int rhs)               { return ((this->x != rhs) || (this->y != rhs) || (this->z != rhs));        }

        inline const point_ti&   operator+=(const int rhs)              { this->x += rhs;   this->y += rhs;   this->z += rhs;   return *this;       }
        inline const point_ti&   operator-=(const int rhs)              { this->x -= rhs;   this->y -= rhs;   this->z -= rhs;   return *this;       }
        inline const point_ti&   operator*=(const int rhs)              { this->x *= rhs;   this->y *= rhs;   this->z *= rhs;   return *this;       }
        inline const point_ti&   operator/=(const int rhs)              { this->x /= rhs;   this->y /= rhs;   this->z /= rhs;   return *this;       }
...
/* Degugging */
inline std::ostream& operator<<(std::ostream &os, const point_ti &p)
{
    return os << p.x << ", " << p.y << ", " << p.z;
}

/* Binary operators */
inline const point_ti operator+(const point_ti &lhs, const int rhs)
{
   return point_ti(lhs.x + rhs, lhs.y + rhs, lhs.z + rhs);
}
...
inline const point_ti operator+(const point_ti &lhs, const point_ti &rhs)
{
   return point_ti(lhs.x + rhs.x, lhs.y + rhs.y, lhs.z + rhs.z);
}
...
inline const point_ti operator+(const int lhs, const point_ti &rhs)
{
   return point_ti(lhs + rhs.x, lhs + rhs.y, lhs + rhs.z);
}
...
/* Other useful functions */
inline point_ti max(const point_ti& a, const point_ti &b)
{
    point_ti ret;
    ret.x = std::max(a.x, b.x);
    ret.y = std::max(a.y, b.y);
...
inline point_ti min(const point_ti& a, const point_ti &b)
{
    point_ti ret;
    ret.x = std::min(a.x, b.x);
    ret.y = std::min(a.y, b.y);
...
inline point_ti max(const point_ti& a, const int b)
{
    point_ti ret;
    ret.x = std::max(a.x, b);
    ret.y = std::max(a.y, b);
...
inline point_ti min(const point_ti& a, const int b)
{
    point_ti ret;
    ret.x = std::min(a.x, b);
    ret.y = std::min(a.y, b);
...
/* Conversions */
inline point_ti::operator point_t()   const   { return point_t(x, y, z);  }
inline point_t::operator point_ti()   const   { return point_ti(x, y, z); }
