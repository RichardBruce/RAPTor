                            ...
                            size_t maxDepth, size_t minLeafSize, Float minAxisSize)
{
    ...
    root.m_triIDs.reserve(nTriangles);
    for(size_t t = 0; t < m_nTriangles; ++t)
	{
		root.m_triIDs.push_back(t);
	}
    root.m_rm = this;
    root.m_id = 0;
...
void RMNode::Create(size_t depth, size_t maxDepth, size_t minLeafSize, Float minAxisSize)
{
    ...
    Vec3<long> * const triangles = m_rm->m_triangles;            
    Vec3<Float> * const vertices = m_rm->m_vertices;
    leftNode.m_triIDs.reserve(m_triIDs.size());
    rightNode.m_triIDs.reserve(m_triIDs.size());
    bool found;
    for(size_t id = 0; id < m_triIDs.size(); ++id)
    {
        f = m_triIDs[id];
        ...
        if (!found)
        {
            for (int k = 0; k < 3; ++k)
            {
                ...
                {
                    rightNode.m_triIDs.push_back(f);
                    break;
                }
            }        
        }
