...
*/
    ...
    for(unsigned long f = 0; f < m_nTriangles; f++)
    {
        ...
        j = m_triangles[f].Y();
        k = m_triangles[f].Z();
        m_graph.m_vertices[f].m_distPoints.push_back(DPoint(i, 0, false, false));
        m_graph.m_vertices[f].m_distPoints.push_back(DPoint(j, 0, false, false));
        m_graph.m_vertices[f].m_distPoints.push_back(DPoint(k, 0, false, false));
        ...
        const long edge_i_j = GetEdgeIndex(i, j);
        auto edge_iter_i_j = std::find(m_graph.m_vertices[f].m_boudaryEdges.begin(), m_graph.m_vertices[f].m_boudaryEdges.end(), edge_i_j);
        if (edge_iter_i_j == m_graph.m_vertices[f].m_boudaryEdges.end())
        {
            m_graph.m_vertices[f].m_boudaryEdges.push_back(edge_i_j);
        }
        
        const long edge_j_k = GetEdgeIndex(j, k);
        auto edge_iter_j_k = std::find(m_graph.m_vertices[f].m_boudaryEdges.begin(), m_graph.m_vertices[f].m_boudaryEdges.end(), edge_j_k);
        if (edge_iter_j_k == m_graph.m_vertices[f].m_boudaryEdges.end())
        {
            m_graph.m_vertices[f].m_boudaryEdges.push_back(edge_j_k);
        }
        
        const long edge_k_i = GetEdgeIndex(k, i);
        auto edge_iter_k_i = std::find(m_graph.m_vertices[f].m_boudaryEdges.begin(), m_graph.m_vertices[f].m_boudaryEdges.end(), edge_k_i);
        if (edge_iter_k_i == m_graph.m_vertices[f].m_boudaryEdges.end())
        {
            m_graph.m_vertices[f].m_boudaryEdges.push_back(edge_k_i);
        }
        ...
        if(m_addFacesPoints)
        {
            ...
            m_graph.m_vertices[f].m_distPoints.push_back(DPoint(-static_cast<long>(f)-1, 0, false, true));
        }
    }
...
#pragma omp parallel for
        ...
        for(long f = 0; f < (long)m_nTriangles; f++)
        {
            ...
            if (faceIndex < m_nTriangles )
            {
                ...
                m_graph.m_vertices[f].m_distPoints.push_back(DPoint(m_nPoints+f, 0, false, true));
            }
...
//        concavity_flat *= std::max(exp(- vol2Surf * 100.0 / (m_scale * m_flatRegionThreshold)) - exp(-1.0), 0.0);
    ...
    if (m_alpha > 0.0)
    {
        std::sort(gV1.m_boudaryEdges.begin(), gV1.m_boudaryEdges.end());
        std::sort(gV2.m_boudaryEdges.begin(), gV2.m_boudaryEdges.end());
        std::vector<unsigned long long> boudaryEdges;
        std::set_symmetric_difference(gV1.m_boudaryEdges.begin(), gV1.m_boudaryEdges.end(), 
                                      gV2.m_boudaryEdges.begin(), gV2.m_boudaryEdges.end(),
                                      std::inserter(boudaryEdges, boudaryEdges.begin()));

        std::vector<unsigned long long>::const_iterator itBE(boudaryEdges.begin());
        std::vector<unsigned long long>::const_iterator itBEEnd(boudaryEdges.end());
    ...
    while ( !m_pqueue.empty() ) 
    {
        ...
        if (!done)
        {
            ...
            if (condition1 || condition2)
            {
                ...
                gV1.m_distPoints.reserve(distPoints.size());
                std::map<long, DPoint>::iterator itDP(distPoints.begin());
                std::map<long, DPoint>::iterator itDPEnd(distPoints.end());
                for(; itDP != itDPEnd; ++itDP) 
                {
                    const DPoint & point = itDP->second;
                    gV1.m_distPoints.push_back(itDP->second);
                }
#else
            ...
            ch->m_mesh.Save("debug.wrl");
                ...
                if (m_alpha > 0.0)
                {
                    ...
                    std::set_symmetric_difference(gV1.m_boudaryEdges.begin(), gV1.m_boudaryEdges.end(), 
                                                  gV2.m_boudaryEdges.begin(), gV2.m_boudaryEdges.end(),
                                                  std::inserter(boudaryEdges, boudaryEdges.begin()));
                    gV1.m_boudaryEdges = boudaryEdges;
                }
