...
class collision_info
{
    public :
        /* CTOR */
        collision_info(simplex *s, const simplex &other_s, const fp_t t, const collision_t type)
            : _s(s), 
              _other_s(&other_s),
              _t(t), 
              _last_t(t),
        ...
        collision_info& update(simplex *s, const simplex &other_s, const fp_t t, const collision_t type) 
        {
            _s.reset(s);
            _other_s = &other_s;
        ...
        {
            _t = std::numeric_limits<fp_t>::max();
            _type = NO_COLLISION;
            return *this;
        }
        ...
        {
            return is_uncertain(_type) ? point_t(0.0, 0.0, 0.0) : _s->normal_of_impact(*_other_s);
        }

        point_t get_point_of_collision() const
        {
            return is_uncertain(_type) ? point_t(0.0, 0.0, 0.0) : _s->center_of_impact(*_other_s, get_normal_of_collision());
        }
    ...
    private :
        ...
        fp_t                        _t;         /* The time of the collision                                            */
        fp_t                        _last_t;    /* The last time seen, not cleared by voiding                           */
