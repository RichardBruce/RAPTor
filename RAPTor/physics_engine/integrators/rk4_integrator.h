...
{
    public :
        point_t project_translation(const aggregate_force &f, const inertia_tensor &i, const point_t &v0, const fp_t dt) const
        {
            point_t v1;
        ...
        point_t project_translation(const aggregate_force &f, const inertia_tensor &i, point_t *const v1, const point_t &v0, const fp_t dt) const
        {
            assert(dt >= 0.0 || !"Error: Negative time step");

            const fp_t half_dt  = dt * 0.5;
            const fp_t sixth_dt = dt * (1.0 / 6.0);

            /* A1 = G(T,S0), B1 = S0 + (DT/2)*A1 */
            const point_t f_1(f.get_force(i, i.center_of_mass(), v0, 0.0));
            ...
            *v1 = v0 + (sixth_dt * (f_1 + ((f_2 + f_3) * 2.0) + f_4)) / i.mass();

            return sixth_dt * (v0 + ((v_1 + v_2) * 2.0) + v_3);
        }


        quaternion_t project_rotation(const aggregate_force &t, const inertia_tensor &i, const quaternion_t &o0, const point_t &w0, const fp_t dt) const
        {
            point_t w1;
        ...
        quaternion_t project_rotation(const aggregate_force &t, const inertia_tensor &i, point_t *const w1, const quaternion_t &o0, const point_t &w0, const fp_t dt) const
        {
            assert(dt >= 0.0 || !"Error: Negative time step");
            ...
            // A1 = G(T,S0), B1 = S0 + (DT/2)*A1
            const quaternion_t q_1(0.5 * quaternion_t(0.0, w0.x, w0.y, w0.z) * o0);
            const point_t t_1(t.get_torque(i, i.center_of_mass(), w0, 0.0));
            ...
            // A2 = G(T+DT/2,B1), B2 = S0 + (DT/2)*A2
            const quaternion_t q_2(0.5 * quaternion_t(0.0, w_1.x, w_1.y, w_1.z) * o_1);
            ...
            // A3 = G(T+DT/2,B2), B3 = S0 + DT*A3
            const quaternion_t q_3(0.5 * quaternion_t(0.0, w_2.x, w_2.y, w_2.z) * o_2);
            ...
            // A4 = G(T+DT,B3), S1 = S0 + (DT/6)*(A1+2*(A2+A3)+A4)
            const quaternion_t q_4(0.5 * quaternion_t(0.0, w_3.x, w_3.y, w_3.z) * o_3);
            const point_t t_4(t.get_torque(i, i.center_of_mass(), w_3, dt));

            const quaternion_t o_4(sixth_dt * (q_1 + ((q_2 + q_3) * 2.0) + q_4));
            const inertia_tensor_view i_4(i, o0 + o_4);
            const point_t l_4(sixth_dt * (t_1 + ((t_2 + t_3) * 2.0) + t_4));
