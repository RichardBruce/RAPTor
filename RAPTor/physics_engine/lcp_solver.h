...
class lcp_solver
{
    public :
        ...
        lcp_solver(const int m_size, const int n_size) :
            _m(new float[m_size * n_size]), _q(new float[m_size]), _w_def(new int [m_size]), _z_def(new int [n_size + 1]), _m_size(m_size), _n_size(n_size)
        {
            // METHOD_LOG;
        ...
        lcp_solver(const lcp_solver &rhs) :
            _m(new float[rhs._m_size * rhs._n_size]), _q(new float[rhs._m_size]), _w_def(new int [rhs._m_size]), _z_def(new int [rhs._n_size + 1]), _m_size(rhs._m_size), _n_size(rhs._n_size)
        {
            // METHOD_LOG;

            memcpy(_m.get(),     rhs._m.get(),     _m_size * _n_size * sizeof(float));
            memcpy(_q.get(),     rhs._q.get(),     _m_size * sizeof(float));
            memcpy(_z_def.get(), rhs._z_def.get(), (_n_size + 1) * sizeof(int));
            memcpy(_w_def.get(), rhs._w_def.get(), _m_size * sizeof(int));
        }

        /* Move CTOR */
        lcp_solver(lcp_solver &&rhs) :
            _m(std::move(rhs._m)), _q(std::move(rhs._q)), _w_def(std::move(rhs._w_def)), _z_def(std::move(rhs._z_def)), _m_size(std::move(rhs._m_size)), _n_size(std::move(rhs._n_size))
        {
            // METHOD_LOG;
        }

        float * initialise_m() { return &_m[_m_size];   }
        float * initialise_q() { return _q.get();       }
    ...
    private :
        ...
        int find_pivot() const
        {
            ...
            for (int i = 0; i < _m_size; ++i)
            {
                if ((_q[i] < q_limit) || ((_q[i] == q_limit) && (i < row)))
                {
                    row = i;
                    q_limit = _q[i];
                }
        ...
        {
            const float divisor = 1.0f / _m[(z_idx * _m_size) + w_idx];
        ...
        void dump() const
        {
            ...
            {
                (*s) << std::setw(10) << ((z > _n_size) ? "w" : "z") << ((z > _n_size) ? (z - _n_size) : z) << "\t";
                return s;
            }, "   q");
        ...
        std::unique_ptr<float []>   _m;
        std::unique_ptr<float []>   _q;
        std::unique_ptr<int []>     _w_def;
        std::unique_ptr<int []>     _z_def;
        const int                   _m_size;
        const int                   _n_size;
