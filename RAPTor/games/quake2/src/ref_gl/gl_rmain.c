...
#ifdef GENERATE_GL_VIDEO
{
    static unsigned int framenr=0;
    static unsigned char filename[100];
    extern void write_png_file(const char *file_name_ptr, unsigned char *png_data, const unsigned int x, const unsigned int y);
    static unsigned char pixels[480][640][3];
    glReadPixels(0, 0, 640, 480, GL_RGB, GL_BYTE, pixels);
    snprintf(filename, 100, "video/%d.png", framenr);
    write_png_file(filename, pixels, 640, 480);
    ++framenr;
}
#endif//GENERATE_GL_VIDEO
}
