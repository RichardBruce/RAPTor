...
void D_WarpScreen (void)
{
...
#ifdef RAPTOR_RAYTRACE
	{
	    extern char* tga_data;
        for (v=0; v<WARP_HEIGHT; ++v)
        {
            for (u=0; u<WARP_WIDTH; ++u)
            {
                unsigned int x = (u*vid.width)/WARP_WIDTH;
                unsigned int y = (v*vid.height)/WARP_HEIGHT;
                unsigned int r = tga_data[(vid.height-y-1)*vid.width*3 + x*3 + 2];
                unsigned int g = tga_data[(vid.height-y-1)*vid.width*3 + x*3 + 1];
                unsigned int b = tga_data[(vid.height-y-1)*vid.width*3 + x*3 + 0];
                unsigned int val = r | (g<<8) | (b<<16);
                val = SDL_MapRGB((SDL_PixelFormat*)vid.pixelformat, r, g, b);
                *((int*)(r_warpbuffer + ((WARP_WIDTH*v + u) * 4))) = val;
            }
        }
	}
#endif
...
#ifdef RAPTOR_RAYTRACE
            rowptr[v] = r_warpbuffer + (WARP_WIDTH * v2 * 4);
#else
...
#ifdef RAPTOR_RAYTRACE
	dest = vid.buffer + r_newrefdef.y * vid.rowbytes + r_newrefdef.x*4;
#else
	...
	for (v=0 ; v<h ; v++, dest += vid.rowbytes)
	{
		...
		{
#ifdef RAPTOR_RAYTRACE
            *((unsigned int*)&dest[(u+0)*4]) = *((unsigned int*)&row[turb[u+0]][col[u+0]]);
            *((unsigned int*)&dest[(u+1)*4]) = *((unsigned int*)&row[turb[u+1]][col[u+1]]);
            *((unsigned int*)&dest[(u+2)*4]) = *((unsigned int*)&row[turb[u+2]][col[u+2]]);
            *((unsigned int*)&dest[(u+3)*4]) = *((unsigned int*)&row[turb[u+3]][col[u+3]]);
#else
