...
#ifdef RAPTOR_RAYTRACE
byte        r_warpbuffer[WARP_WIDTH * WARP_HEIGHT * 4];
#else
...
#ifdef RAPTOR_RAYTRACE
/**
 * A slightly modified version of the function "R_BuildPolygonFromSurface(msurface_t *fa)" in r_poly.c.
 * We use it to get some useful information from a surface.
 */
polydesc_t get_polydesc(msurface_t *fa)
{
    ...
    {
        lindex = r_worldmodel->surfedges[fa->firstedge + i];

        if (lindex > 0)
        {
            r_pedge = &pedges[lindex];
            vec = r_worldmodel->vertexes[r_pedge->v[0]].position;
        }
        else
        {
            r_pedge = &pedges[-lindex];
            vec = r_worldmodel->vertexes[r_pedge->v[1]].position;
        }
    }

    VectorCopy( fa->texinfo->vecs[0], ret.vright );
    VectorCopy( fa->texinfo->vecs[1], ret.vup );
    VectorCopy( fa->plane->normal, ret.vpn );
    VectorCopy( r_origin, ret.viewer_position );

    if ( fa->flags & SURF_PLANEBACK )
    {
        VectorSubtract( vec3_origin, ret.vpn, ret.vpn );
    }

    ret.pixels       = fa->texinfo->image->pixels[0];
    ret.pixel_width  = fa->texinfo->image->width;
    ret.pixel_height = fa->texinfo->image->height;
    tmins[0] = fa->texturemins[0];
    tmins[1] = fa->texturemins[1];

    ret.s_offset = fa->texinfo->vecs[0][3] - tmins[0];
    ret.t_offset = fa->texinfo->vecs[1][3] - tmins[1];
...
/**
 * Passes triangles, textures, lights to the raytracer
 */
void raptor_prepare_triangles()
{
    ...
    // Loop over all entities and record surface offsets for all brush models
    for (ent=0; ent<r_newrefdef.num_entities; ++ent)
    {
        if (!r_newrefdef.entities[ent].model)
            continue;
        if (r_newrefdef.entities[ent].model->nummodelsurfaces == 0)
            continue;
        if (r_newrefdef.entities[ent].flags & RF_BEAM )
            continue;
        if (r_newrefdef.entities[ent].model->type != mod_brush)
            continue;

        for (n=r_newrefdef.entities[ent].model->firstmodelsurface; n<r_newrefdef.entities[ent].model->firstmodelsurface+r_newrefdef.entities[ent].model->nummodelsurfaces; ++n)
        {
            msurface_t* surf = &r_newrefdef.entities[ent].model->surfaces[n];
            raptor_set_surf_offset((void*)surf, r_newrefdef.entities[ent].origin, r_newrefdef.entities[ent].angles);
        }
    }

    // Add all dynamic lights to raytracer
    for (i=0; i<r_newrefdef.num_dlights; ++i)
    {
        float intensity = r_newrefdef.dlights[i].intensity;
        float falloff;
        if (intensity < 0) intensity = -intensity; // Just imitate the Quake II source...
        if (intensity > 256) intensity = 256; // TODO: find out the true intensity range
        falloff = (256.0f - intensity)/25600.0f;

        VectorSubtract(r_newrefdef.dlights[i].origin, r_origin, local);
        VectorScale(local, 0.90f, local); // Workaround for handling the light being located inside a projectile
        TransformVector(local, transformed);

        raptor_push_light(transformed,
                          r_newrefdef.dlights[i].color[0]*255,
                          r_newrefdef.dlights[i].color[1]*255,
                          r_newrefdef.dlights[i].color[2]*255,
                          falloff);
    }

/************************************************************************************/
/**  Alternative ways of looping over the world surfaces, kept here for reference. **/
/***                                                                              ***/
//    for (n=r_worldentity.model->firstmodelsurface; n<r_worldentity.model->firstmodelsurface+r_worldentity.model->nummodelsurfaces; ++n)
//    for (n=r_worldmodel->firstnode; n<(r_worldmodel->firstnode+r_worldmodel->numnodes); ++n)
//        mnode_t* node = &r_worldmodel->nodes[n];
//            if (node->children[c]->contents!=-1 && !(node->children[c]->contents & CONTENTS_SOLID))
//                mleaf_t* leaf = (mleaf_t*)node->children[c];
//                for (i=0; i<leaf->nummarksurfaces; ++i)
//                  msurface_t* surf = *(leaf->firstmarksurface+i);

    // Loop over all the world nodes and all surfaces in each node
    for (n=r_worldmodel->firstnode; n<(r_worldmodel->firstnode+r_worldmodel->numnodes); ++n)
    {
        mnode_t *node = &r_worldmodel->nodes[n];

        for (c=node->firstsurface; c<node->firstsurface+node->numsurfaces; ++c)
        {
            msurface_t       *surf   = &r_worldmodel->surfaces[c];
            surface_offset_t  so     = raptor_get_surf_offset(surf);
            vec3_t            centre = {0, 0, 0}; // Centre of surface

#ifdef RAPTOR_TEXTURE_MAPPING_ON
            polydesc_t pdesc = get_polydesc(surf);
#endif

            // Skip transparent surfaces and surfaces that shouldn't be drawn.
            if (surf->texinfo->flags & (SURF_TRANS33|SURF_TRANS66|SURF_NODRAW))
                continue;

            // If the current surface is a skybox, initialize the RAPTor skybox.
            if (surf->texinfo->flags & SURF_SKY)
            {
                float skybox_p[2][3] = {{ RAPTOR_SKYBOX_SIZE,  RAPTOR_SKYBOX_SIZE,  RAPTOR_SKYBOX_SIZE},
                                        {-RAPTOR_SKYBOX_SIZE, -RAPTOR_SKYBOX_SIZE, -RAPTOR_SKYBOX_SIZE}};
                byte* skybox_textures[6];
                unsigned int skybox_widths[6];
                unsigned int skybox_heights[6];
                extern mtexinfo_t r_skytexinfo[6];

                for (i=0; i<6; ++i)
                {
                    skybox_textures[i] = r_skytexinfo[i].image->pixels[0];
                    skybox_widths[i]   = r_skytexinfo[i].image->width;
                    skybox_heights[i]  = r_skytexinfo[i].image->height;
                }

                raptor_init_skybox(skybox_p[0], skybox_p[1], skybox_textures, skybox_widths, skybox_heights);
            ...
            {
                int axis;
                mvertex_t current_vertex;
                vec3_t offset;
                vec3_t angles;
                vec3_t pos;

                lindex = r_worldmodel->surfedges[surf->firstedge + j];
                vert = 0;

                // If lindex is negative, then the edge direction is inverted.
                if (lindex<0)
                {
                    lindex = -lindex;
                    vert = 1;
                ...
                current_vertex = r_worldmodel->vertexes[r_worldmodel->edges[lindex].v[vert]];
                VectorCopy(current_vertex.position, pos);

                // Get the offset (if present) for the current surface. The offset
                // was previously stored above, and is used for moving entities such
                // as e.g. doors and lifts.
                offset[0] = so.offset[0];
                offset[1] = so.offset[1];
                offset[2] = so.offset[2];
                angles[0] = so.angles[0];
                angles[1] = so.angles[1];
                angles[2] = so.angles[2];
                ...
                for (axis=0; axis<3; ++axis)
                {
                    if (angles[axis])
                    {
                        ...
                        raptor_rotate_vector(&pos, axis, angles[axis]);
                ...
                VectorAdd(local, offset, local);
                TransformVector(local, transformed);

                raptor_push_vertex(transformed[0], transformed[1], transformed[2]);

#ifdef RAPTOR_TEXTURE_MAPPING_ON
{
                // Calculate corresponding texture coordinates for current vertex.
                float uv[2];
                for (k=0; k<2; k++)
                {
                    int wid = surf->texinfo->image->width;
                    int hgt = surf->texinfo->image->height;
                    // s,t are calculated in the same way as in CalcSurfaceExtents() in r_model.c.
                    float val = current_vertex.position[0] * surf->texinfo->vecs[k][0] +
                                current_vertex.position[1] * surf->texinfo->vecs[k][1] +
                                current_vertex.position[2] * surf->texinfo->vecs[k][2] +
                                surf->texinfo->vecs[k][3];
                    // Map it to range [0, 1]
                    if (k==0)
                        uv[0] = val/(float)wid;
                    else
                ...
                raptor_push_tex_vertex(uv[0], uv[1]);
}
#endif//RAPTOR_TEXTURE_MAPPING_ON

                VectorAdd(centre, transformed, centre);
            }

            VectorScale(centre, 1.0f/surf->numedges, centre);

#ifdef RAPTOR_TEXTURE_MAPPING_ON
{
            vec3_t dummy = {0, 0, 0};
            raptor_push_surface_texture(pdesc.pixels, pdesc.pixel_width, pdesc.pixel_height,
                                        surf->texinfo->image->name,
                                        dummy, dummy, dummy,
                                        centre);
            raptor_create_triangles(1);
}
#else
            raptor_create_triangles(0);
#endif
        }
    }
}
...
#ifdef RAPTOR_RAYTRACE
    raptor_init((unsigned int*)vid.buffer, vid.pixelformat, vid.width, vid.height);
	raptor_prepare_triangles();
#endif
...
#ifdef RAPTOR_RAYTRACE
{
	...
	static vec3_t n     = {0, 0, 1};
	static vec3_t up    = {0, 1, 0};
    vec3_t vx, m0, m1, m2;
    float angle;
    float len;

    TransformVector(right, m0);
    TransformVector(up,    m2);
    TransformVector(n,     m1);
    angle = acos((m0[0]+m1[1]+m2[2]-1.0f)/2.0f);
    len   = sqrt(pow(m2[1]-m1[2], 2.0f) + pow(m0[2]-m2[0], 2.0f) + pow(m1[0]-m0[1], 2.0f));
    vx[0] = (m2[1]-m1[2])/len;
    vx[1] = (m0[2]-m2[0])/len;
    vx[2] = (m1[0]-m0[1])/len;

    raptor_raytrace(0, 0, 0,
...
#ifdef RAPTOR_RAYTRACE
// TODO: This function can be removed, along with the rest of the interface
// for adding lights, since we now read lights from the raptor cfg file
// instead of from the map file.
void add_light(float pos[3], float r, float g, float b, float intensity)
{
    float falloff;
    if (intensity>256)
        intensity = 256;
    ...
    if (falloff<0)
        falloff = 0;
//    printf("RAPTOR ADDING A LIGHT at %f %f %f, r=%f g=%f b=%f falloff=%f!\n", pos[0], pos[1], pos[2], r, g, b, falloff);
    return;
/*    light_desc_t l;
    l.pos[0] = pos[0];
    l.pos[1] = pos[1];
    ...
    l.b = b;
    l.falloff = falloff;
    raptor_push_light_permanent(l);
*/
}

add_light_func_ptr_t get_add_light(void)
{
    return add_light;
}

raptor_refexport_t GetRAPTorRefAPI()
{
    raptor_refexport_t re;
    re.GetAddLightFunc = get_add_light;
    re.RaptorLoadLevelFunc = raptor_load_level;
    return re;
}
#endif//RAPTOR_RAYTRACE
