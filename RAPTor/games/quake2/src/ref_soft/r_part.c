...
#ifdef RAPTOR_RAYTRACE
    {
        float locallen = sqrt(local[0]*local[0]+local[1]*local[1]+local[2]*local[2]);
        float v0[3], v1[3], v2[3];
        float r = (float)((d_8to24table[color] >> 0 ) & 0xFF);
        float g = (float)((d_8to24table[color] >> 8 ) & 0xFF);
        float b = (float)((d_8to24table[color] >> 16) & 0xFF);
        transformed[0]/=locallen;
        transformed[1]/=locallen;
        v0[0] = transformed[0];
        v0[1] = transformed[1];
        v0[2] = transformed[2];
        v1[0] = transformed[0] + 2.5f;
        v1[1] = transformed[1] + 2.5f;
        v1[2] = transformed[2];
        v2[0] = transformed[0] + 2.5f;
        v2[1] = transformed[1];
        v2[2] = transformed[2];
        // Set raptor_fake_light so that the triangle is specified as a light without actually being associated with a light source.
        raptor_push_triangle(v0, v1, v2, 0, r, g, b, raptor_fake_light);
        v1[0] -= 2.5f;
        v2[1] += 2.5f;
        raptor_push_triangle(v0, v1, v2, 0, r, g, b, raptor_fake_light);
        return;
