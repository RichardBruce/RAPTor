...
#ifdef RAPTOR_RAYTRACE
	GetRAPTorGameAPI_t GetRAPTorGameAPI;
#endif//RAPTOR_RAYTRACE
...
#ifdef RAPTOR_RAYTRACE
{
	extern add_light_func_ptr_t AddLight;
	GetRAPTorGameAPI = (void *)dlsym (game_library, "GetRAPTorGameAPI");
    if (!GetRAPTorGameAPI)
    {
        Sys_UnloadGame ();
        return NULL;
    ...
    assert(AddLight != 0);
    raptor_game_export_t raptor_ge = GetRAPTorGameAPI();
    raptor_ge.SetAddLightFunc(AddLight);
}
#endif//RAPTOR_RAYTRACE
