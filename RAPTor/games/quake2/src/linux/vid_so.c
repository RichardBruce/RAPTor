...
#ifdef RAPTOR_RAYTRACE
	GetRAPTorRefAPI_t GetRAPTorRefAPI;
#endif//RAPTOR_RAYTRACE
	char	fn[MAX_OSPATH];
	char	*path;
	struct stat st;
...
#ifdef RAPTOR_RAYTRACE
    if ( ( GetRAPTorRefAPI = (void *) dlsym( reflib_library, "GetRAPTorRefAPI" ) ) == 0 )
        Com_Error( ERR_FATAL, "dlsym failed on %s", name );
    raptor_re = GetRAPTorRefAPI();
	AddLight = raptor_re.GetAddLightFunc();
#endif//RAPTOR_RAYTRACE
