...
{
    public :
        planar_mapper(const boost::shared_array<float> &im, const point_t &c, const point_t &n, const point_t &s, 
            ...
            u_off(u_off), v_off(v_off)
            {
                ...
                this->u = -this->u;
                // std::cout << "n vec: " << this->n << std::endl;
                // std::cout << "u vec: " << this->u << std::endl;
                // std::cout << "v vec: " << this->v << std::endl;
            };

        planar_mapper(const boost::shared_array<float> &im, const point_t &u, const point_t &v, const point_t &c, const point_t &n, const point_t &s, 
            const unsigned cpp, const unsigned w, const unsigned h, const texture_wrapping_mode_t uw, const texture_wrapping_mode_t vw,
            const int u_off = 0, const int v_off = 0, const int u_max = -1, const int v_max = -1)
