...
int mg_defuhand(ac, av)        /* default handler for unknown entities */
int    ac;
char    **av;
{
...
char    **av;
{
    ...
    for (i = 1; i <= 2*mg_nqcdivs; i++) {
        double theta = i*(PIE/2)/mg_nqcdivs;
    ...
    else            /* open */
    {
        ...
        for (i = 1; i <= 4*mg_nqcdivs; i++) {
            ...
            if ((rv = mg_handle(MG_E_VERTEX, 4, v2ent)) != MG_OK)
                return(rv);
            for (j = 0; j < 3; j++) {
                double d = u[j]*cos(theta) + v[j]*sin(theta);
...
char    **av;
{
    static char    p3[3][24], n3[3][24], n4[3][24];
    ...
    else            /* quads */
    {
        ...
        v1ent[3] = "_cv4";
        if (warpconends) {        /* hack for e_sph and e_torus */
            d = atan(n1off) + (PIE/4)/mg_nqcdivs;
            if (d >= PIE/2-FTINY)
                n1off = FHUGE;
            else
                n1off = tan(atan(n1off)+(PIE/4)/mg_nqcdivs);
        }
        for (j = 0; j < 3; j++) {
...
put_cspec()            /* put out current color spectrum */
{
    ...
    if (mg_ehand[MG_E_CSPEC] != c_hcolor)
    {
        ...
        newav[1] = wl[0];
        newav[2] = wl[1];
        double sf = (double)C_CNSS / c_ccolor->ssum;
