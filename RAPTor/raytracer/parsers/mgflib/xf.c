...
long comp_xfid(xfm)            /* compute unique ID from matrix */
MAT4    xfm;
{
    ...
    long xid = 0;            /* compute unique transform id */
    for (int i = 0; i < sizeof(MAT4) / sizeof(unsigned short); ++i)
    {
...
void xf_xfmpoint(v1, v2)        /* transform a point by the current matrix */
FVECT    v1, v2;
{
    if (xf_context == NULL)
    {
...
void xf_xfmvect(v1, v2)        /* transform a vector using current matrix */
FVECT    v1, v2;
{
    if (xf_context == NULL) {
        VCOPY(v1, v2);
...
void xf_rotvect(v1, v2)        /* rotate a vector using current matrix */
FVECT    v1, v2;
{
    xf_xfmvect(v1, v2);
    if (xf_context == NULL)
