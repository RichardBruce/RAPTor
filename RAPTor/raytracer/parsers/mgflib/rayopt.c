                 ...
                 TriList2 **best_loc)
{
    ...
    *best_loc  = NULL;
    ...
    for (t = gnode->index[axis]->next; t != gnode->index[axis]; t = t->next) {
        ...
        float new_index = (cnt * area1) + ((gnode->obj_cnt - cnt) * area2);
                  ...
                  GroupTree **group_a, GroupTree **group_b)
{
    ...
    if (!quiet_mode) {
        printf ("Adding bounds (%u)\r", tot_bounds);
        fflush(stdout);
    }
...
void write_pov10_tree (FILE *f, GroupTree *gnode, int level)
{
    ...
    if (gnode->child != NULL) {
        ...
        Triangle *first_tri = gnode->index[0]->next->tri;
        int one_texture = 1;
...
void write_mgf_transform (FILE *f, Matrix matrix)
{
    ...
    if (fabs(scale[X] - 1.0) > 0.001 || fabs(scale[Y] - 1.0) > 0.001 ||
        ...
        float ascale = sqrt((scale[X]*scale[X] + scale[Y]*scale[Y] +
                        scale[Z]*scale[Z])/3.0);
        fprintf (f, " -s %.3f\n", ascale);
    }
                                                /* add rotation */
...
{
    int i;
    float max_v = -MAXFLOAT;
    for (i = 0; i < 3; ++i)
    {
        float val = vtable[tri->vert[i]][axis];
        if (val > max_v)
        {
            max_v = val;
        }
...
float avg_vertex (Triangle *tri, int axis)
{
    float avg = (vtable[tri->vert[0]][axis] + vtable[tri->vert[1]][axis] +
