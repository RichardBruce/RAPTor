...
void process_args (int argc, char *argv[])
{
    ...
    if (env_opt != NULL) {
        char *option = parse_string (env_opt);
...
void write_pov10_material (FILE *f, MatProp *m)
{
    ...
    if (m->reflection > 0.0) {
        float spec = (m->specular.red + m->specular.green + m->specular.blue)/3.0;
        fprintf (f, "    reflection %.3f\n", spec * m->reflection);
    }

    if (m->transparency > 0.0) {
        float red   = m->diffuse.red;
        float green = m->diffuse.green;
        float blue  = m->diffuse.blue;
...
void read_frame (char *filename, int frame_no)
{
    ...
    while (fgets (string, 256, f) != NULL) {
        char *token = parse_string (string);
