...
char    **av;
{
    ...
    case MG_E_MATERIAL:    /* get/set material context */
        ...
        if (ac == 2) {        /* reestablish previous context */
            if (c_cmaterial == NULL)
                return(MG_EUNDEF);
            c_cmaterial->clock = 1; /* Added to let the translater know when a material has been re-established */
            return(MG_OK);
        }
        if (av[2][0] != '=' || av[2][1])
            return(MG_ETYPE);
        if (c_cmaterial == NULL) {    /* create new material */
            ...
            c_cmname = lp->key;
            c_cmaterial = (C_MATERIAL *)lp->data;
            c_cmaterial->clock = 1; /* Added to let the translater know when a material has been created */
