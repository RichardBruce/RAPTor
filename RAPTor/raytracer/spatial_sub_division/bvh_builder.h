...
class bvh_builder
{
    public :
        ...
        bvh_builder(const float alpha = 0.4f, const float delta = 20.0f, const float max_leaf_sah_factor = 0.0002f, const int max_node_size = MAX_BVH_NODE_SIZE, const int max_down_phase_depth = 35) :
        _primitives(nullptr), _nodes(nullptr), _alpha(alpha), _epsilon(1e-8f), _delta(delta), _max_leaf_sah_factor(max_leaf_sah_factor), _max_node_size(max_node_size), _max_down_phase_depth(max_down_phase_depth), _depth(0), _rows(0), _next_node(0) {  }
