...
inline int bvh::find_leaf_node(const frustrum &r, bvh_stack_element *const entry_point, bvh_stack_element **const out, unsigned int size) const
{
    // bvh_stack_element *exit_point = *out;
    
    // /* Pull out indices of the current node */
    // const int cur_idx = entry_point->idx;
    // int bvh_block   = block_index(cur_idx);
    // int bvh_node    = node_index(cur_idx);
    ...
    // while (true)
    // {
    //     float near_split = (*_bvh_base)[bvh_block].get_node(bvh_node)->get_left_split();
    //     float far_split  = (*_bvh_base)[bvh_block].get_node(bvh_node)->get_right_split();
        
    //     int far_idx;
    //     int near_idx = (*_bvh_base)[bvh_block].get_siblings(&far_idx, bvh_block, bvh_node);

    //     if (bvh_node == 0)
    //     {
    //         const int child_block = block_index((*_bvh_base)[bvh_block].get_child_block());
    //         _mm_prefetch((char *)&(*_bvh_base)[child_block], _MM_HINT_T0);
    //         _mm_prefetch((char *)&(*_bvh_base)[child_block + 1], _MM_HINT_T0);
    //         _mm_prefetch((char *)&(*_bvh_base)[child_block + 2], _MM_HINT_T0);
    //         _mm_prefetch((char *)&(*_bvh_base)[child_block + 3], _MM_HINT_T0);
    //         _mm_prefetch((char *)&(*_bvh_base)[child_block + 4], _MM_HINT_T0);
    //         _mm_prefetch((char *)&(*_bvh_base)[child_block + 5], _MM_HINT_T0);
    //         _mm_prefetch((char *)&(*_bvh_base)[child_block + 6], _MM_HINT_T0);
    //         _mm_prefetch((char *)&(*_bvh_base)[child_block + 7], _MM_HINT_T0);
    //     }

    //     switch ((*_bvh_base)[bvh_block].get_split_axis(bvh_node))
    //     {
    ...
    //             *out = exit_point;
    //             entry_point->idx = bvh_index(bvh_block, bvh_node);
    //             entry_point->u = u;
    //             entry_point->l = l;
    //             return 1;
    //         }
    ...
    //                 bvh_block   = block_index(far_idx);
    //                 bvh_node    = node_index(far_idx);
    //             }
    ...
    //                 bvh_block   = block_index(near_idx);
    //                 bvh_node    = node_index(near_idx);
    //             }
...
{
//     _mm_prefetch((char *)&(*_bvh_base)[0], _MM_HINT_T0);
...
//         auto leaf_node = (*_bvh_base)[block_index(entry_point.idx)].get_node(node_index(entry_point.idx));
...
// //            {
// //                this->bvh_frustrum_find_nearest_object(&r[i], &i_o[i << LOG2_SIMD_WIDTH], &h[i], entry_point, exit_point, size/SPLIT_PACKET_DIVISOR);
// //            }
//             for (int i = 0; i < size; ++i)
//             {
...
inline bool bvh::find_leaf_node(const packet_ray &r, bvh_stack_element *const entry_point, bvh_stack_element **const out, const vfp_t *const i_rd) const
{
    // bvh_stack_element *exit_point = *out;
    ...
    // while (true)
    // {
    //     const vfp_t near_split((*_bvh_base)[bvh_block].get_node(bvh_node)->get_left_split());
    //     const vfp_t far_split ((*_bvh_base)[bvh_block].get_node(bvh_node)->get_right_split());
...
void bvh::find_nearest_object(const packet_ray *const r, const triangle **const i_o, packet_hit_description *const h) const
{
    ...
    //     if (cmd)
    //     {
    //         (*_bvh_base)[block_index(entry_point.idx)].get_node(node_index(entry_point.idx))->test_leaf_node_nearest(r, i_o, h, 1);
    //     }
...
vfp_t bvh::found_nearer_object(const packet_ray *const r, const vfp_t &t, bvh_stack_element entry_point, bvh_stack_element *exit_point) const
{
    ...
    //     if (cmd)
    //     {
    //         (*_bvh_base)[block_index(entry_point.idx)].get_node(node_index(entry_point.idx))->test_leaf_node_nearer(r, &closer, t, &h);
...
inline bool bvh::find_leaf_node(const ray &r, bvh_stack_element *const entry_point, bvh_stack_element **const out, const point_t &i_rd) const
{
    // bvh_stack_element *exit_point = *out;
...
triangle* bvh::find_nearest_object(const ray *const r, hit_description *const h) const
{
    ...
    //         triangle *intersecting_object = (*_bvh_base)[block_index(entry_point.idx)].get_node(node_index(entry_point.idx))->test_leaf_node_nearest(r, &nearest_hit);
...
bool bvh::found_nearer_object(const ray *const r, const float t) const
{
    ...
    //         const bool closer = (*_bvh_base)[block_index(entry_point.idx)].get_node(node_index(entry_point.idx))->test_leaf_node_nearer(r, t);
